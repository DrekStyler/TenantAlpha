...
export function AIChatWindow({ dealId, calculationResults }: AIChatWindowProps) {
  ...
  return (
    <div className="flex flex-col" style={{ minHeight: 320 }}>
      <div className="flex-1 space-y-3 overflow-y-auto py-2" style={{ maxHeight: 400 }}>
        {messages.length === 0 && (
            ...
            <div className="flex flex-wrap gap-2">
              {STARTERS.map((s) => (
                <button
                  key={s}
                  onClick={() => setInput(s)}
                  className="rounded-full border border-navy-200 px-3 py-1.5 text-xs text-navy-600 transition-colors hover:border-navy-400 hover:bg-navy-50"
                >
                  {s}
                </button>
              ))}
      ...
      >
        <textarea
          ...
          className="flex-1 resize-none rounded-lg border border-navy-200 px-3 py-2 text-sm text-navy-800 placeholder:text-navy-400 focus:border-navy-500 focus:outline-none focus:ring-1 focus:ring-navy-500"
        />
        <Button
          type="submit"
          size="sm"
